...
- [x] Create GitHub Actions workflow `.github/workflows/ci.yml` (typecheck + test + lint across Node 18, 20, 22)
- [x] ESLint + Prettier clean, coverage ≥85% on `typed/evaluator/infer`
- [x] README examples verified, `--help` snapshot, `npm pack` dry-run, CI green
- [x] Optional `0x` self-profile notes (no jq shootout claims)
- [x] Done when: fresh-clone `npm ci && npm test && npm run build` passes on CI
